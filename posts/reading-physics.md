...
What's actually going on here is that vectors and tensors really belong to special linear
spaces that are made up of *functions*. The nature of these objects takes a bit of formal
background to explain precisely,[^1]  and it is covered in books about differential
geometry and related areas. For the physics people it's easier and quicker to just write
down the rules and not think about it. I will not go into this any more now, except to
note how similar this is to reading code in a language with no types.
